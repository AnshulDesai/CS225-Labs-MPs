...
void SCHashTable<K, V>::resizeTable()
{
    ...
    size_t sizeNew = findPrime(2 * size);
    list<pair<K, V>> * tempTable = new list<pair<K, V>>[sizeNew];
    
    for(size_t i = 0; i < size; i++){
      for(it = table[i].begin(); it != table[i].end(); it++){
        size_t tempIndex = hash(it->first, sizeNew);
        tempTable[tempIndex].push_front({it->first, it->second});
      }
    }
      size = sizeNew;
      delete [] table;
