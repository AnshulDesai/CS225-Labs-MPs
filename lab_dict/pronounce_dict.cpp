...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    transform(secondWord.begin(), secondWord.end(), secondWord.begin(), ::toupper);
    if(dict.count(firstWord) == 0 || dict.count(secondWord) == 0){
      return false;
    }

    vector<string> firstVectString = dict.at(firstWord);
    vector<string> secondVectString = dict.at(secondWord);

    if(firstVectString == secondVectString){
      return true;
    }
