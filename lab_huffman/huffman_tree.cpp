                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
     TreeNode * boonk = NULL;
     // quick check for if the merge is empty
     if (singleQueue.empty() && !mergeQueue.empty()) {
       boonk = mergeQueue.front();
       mergeQueue.pop();
     }
     // quick check for if the single is empty
     if (mergeQueue.empty() && !singleQueue.empty()) {
       boonk = singleQueue.front();
       singleQueue.pop();
     }
     // quick check for when both queues are not empty
     if (!singleQueue.empty() && !mergeQueue.empty()) {
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     for (unsigned i=0; i<frequencies.size(); i++) {
       boonk = new TreeNode(frequencies[i]);
       singleQueue.push(boonk);
	   }
     TreeNode * alpha;
     TreeNode * beta;
     // Next, until there is only one node on the two queues
     while (singleQueue.size() + mergeQueue.size() != 1) {
       // remove the two smallest entries from the two queues
       alpha = removeSmallest(singleQueue, mergeQueue);
       if (singleQueue.size() + mergeQueue.size() != 0) {
         beta = removeSmallest(singleQueue, mergeQueue);
       }
       // Then, create a new internal node with these nodes as children
       boonk = new TreeNode(alpha -> freq.getFrequency() + beta -> freq.getFrequency());
       boonk -> left = alpha;
       boonk -> right = beta;
       mergeQueue.push(boonk);
     }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
     ...
     */
     ...
     if (bfile.getNextBit() == true) {
       TreeNode * temp = new TreeNode(Frequency(bfile.getNextByte(), 0));
       return temp;
     }
     else {
