...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    bool test = (elements.size() == 0) || (elements[0] > val);
    if (test) {
      return 0;
    }

    bool test2 = (val > elements[elements.size() - 1]);
    if (test2) {
      return elements.size();
    }

    for (size_t check = 0; check < elements.size(); check++) {
      if (elements[check] == val || elements[check] > val) {
        return check;
      }
    }
