...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    for (size_t check = 0; check < subroot->elements.size(); check++) {
      if (subroot->elements[check].key == pair.key) {
        return;
      }
    }

    if (subroot->is_leaf) {
      auto tempIt = subroot->elements.begin() + first_larger_idx;
      subroot->elements.insert(tempIt, pair);
      return;
    }
