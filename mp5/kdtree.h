...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     int getMedian(int begin, int end) const;

     void swap(vector<Point<Dim>>& points, int first, int second);

     void clearTree(typename KDTree<Dim>::KDTreeNode * rootNode);

     Point<Dim> find(int begin, int end, int dimen, const Point<Dim> & best, const Point<Dim> & query) const;

     int medIndex(int begin, int end, int dimen, int pivot);

     void quickSelect(int begin, int end, int dimen, int swag);

     void buildTree(int begin, int end, int dimen);

     KDTreeNode * constructTree(int begin, int end);
};
