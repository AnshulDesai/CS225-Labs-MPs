...
std::vector<Edge> NimLearner::playRandomGame() const {
    ...
    while (g_.getAdjacent(temp).size() != 0) {
      vector<Vertex> adjacentVertices = g_.getAdjacent(temp);
      int jump = rand() % adjacentVertices.size();
      Vertex nextVertex = adjacentVertices[jump];
      path.push_back(g_.getEdge(temp, nextVertex));
      temp = nextVertex;
    }
  return path;
}
...
void NimLearner::updateEdgeWeights(const std::vector<Edge> & path) {
 ...
 for (int i = path.size() - 1; i >= 0; i--) {
   Edge e = path[i];
   int tempWeight = g_.getEdgeWeight(e.source, e.dest);
   g_.setEdgeWeight(e.source, e.dest, tempWeight + temp);
   temp = temp * -1;
 }
}
